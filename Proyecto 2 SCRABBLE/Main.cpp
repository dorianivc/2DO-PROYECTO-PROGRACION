...
int main() {
	...
	for (int y = 0; y < 13; y++) {
		...
		else
		{
			...
			Fichas* aux = juegoPrincipal->getA()->getFichasJugador()->getFichaDeLista(entrada);
			if (aux != NULL)
			{
				...
				if (aux->getLetra() == entrada && contador > 0)
				{
					...
					if (aux->getLetra() != entrada || aux == NULL)
					{
			...
			if (seleccionA1 == 2)
			{
				...
				if (seleccionA3 == 1)
				{
					...
					if (seleccionA4 == 1)
					{
						...
						{
							delete entrante;
							entrante = new Palabra(fichasJugada1);
							cout << "Digite la letra de la columna donde quiere poner la palabra" << endl;
							char columna;
							cin >> columna;//validaciones no puede ser numero, letra ni mayor a 13
							juegoPrincipal->retornaValor(columna);
					...
					{
						{
							...
							for (int y = 0; y < 13; y++) {
								...
								else
								{
									...
									Fichas* aux = juegoPrincipal->getB()->getFichasJugador()->getFichaDeLista(entrada);
									if (aux != NULL) 
									{
										if (aux->getLetra() == entrada && contador == 0)
										{
